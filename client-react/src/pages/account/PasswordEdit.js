...
const PasswordEdit = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (formData.password === formData.password2) {
      axios
        .put(`/api/user/${user.userId}`, { password: formData.password })
        .then((response) => {
          ...
          <div class="detailform-cont">
            <Form onSubmit={handleSubmit}>
              ...
              <FormGroup as={Row} controlId="password2">
                <Col sm={buffer} />
                <FormLabel column sm={keyWidth}>
                  <span class="font-weight-bold">Re-Enter New Password: </span>
                </FormLabel>
                <Col sm={valueWidth}>
                  <FormControl
                    type="Password"
              ...
              <Row>
                ...
                <Col sm={valueWidth + 1}>
                  ...
                  {changeStatus === "Hang on, changing your password..." ? (
                    <Alert variant="secondary">{changeStatus}</Alert>
                  ) : (
                    ""
                  )}
                  {errorMsg ? (
                    <Alert variant="danger">{showErrors()}</Alert>
                  ) : (
                    ""
                  )}
                </Col>
